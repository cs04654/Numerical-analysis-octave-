...
function Jacobi = Jacobi(A,b,x0,eta)
...
detM = det(M);
if (detM == 0)
  error('DETERMINANT OF M IS detM = %f = 0(JACOBI NOT APPLICABLE) \n', detM);
else
  fprintf('DETERMINANT OF M IS detM = %f /= 0 (JACOBI IS APPLICABLE) \n', detM);
end
fprintf('MATRIX N = L+U: \n');
disp(N);
...
if (nT1 >= 1)
  ...
  if (nTinf >= 1)
    ...
    if (specrad >= 1)
      error('[17] CHECKING SPECTRAL RADIUS OF T IS %d >= 1 ### JACOBI IS NOT CONVERGENT \n', specrad);
    else
      fprintf('[3] CHECKING SPECTRAL RADIUS OF T IS %d < 1 ### JACOBI IS CONVERGENT \n', specrad);
    end
  else
    fprintf('[3] CHECKING L_inf-norm of T:  ||T||_inf = %f < 1 (JACOBI METHOD CONVERGENCE GUARANTEED) \n', nTinf);
  end
else
  fprintf('[3] CHECKING L1-norm of T:  ||T||_1 = %f < 1 (JACOBI METHOD CONVERGENCE GUARANTEED) \n', nT1);
end
fprintf('\n');
